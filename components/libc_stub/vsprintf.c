...
int sniprintf(char *buf, size_t size, const char *fmt, ...)
{
	va_list args;
	int i;
...
#define VSPRINTF_BUFFER_MAX_SIZE 4096
...
int sprintf(char *buf, const char *fmt, ...)
{
	...
	va_start(args, fmt);
	i = vsnprintf(buf, ~(size_t)0, fmt, args);
	va_end(args);
...
static int __vprint(int fd, const char *fmt, va_list argp)
{
	static char string[VSPRINTF_BUFFER_MAX_SIZE];
    int len;

	len = vsnprintf(string, VSPRINTF_BUFFER_MAX_SIZE, fmt, argp);
	if (len <=0 ) {
		return -1;
	}
	...
	return _write_r(_impure_ptr, fd, string, len);
}
...
int printf(const char *fmt, ...)
{
    ...
    va_start(argp, fmt);
    ret = __vprint(STDOUT_FILENO, fmt, argp);
    va_end(argp);
...
//int vfiprintf(FILE *stream, const char *format, va_list argp)
//{
//    return __vprint(fileno(stream), format, argp);
//}
//
...
//    ret = vfiprintf(stream, format, argp);
//    va_end(argp);
//
//    return ret;
//}
