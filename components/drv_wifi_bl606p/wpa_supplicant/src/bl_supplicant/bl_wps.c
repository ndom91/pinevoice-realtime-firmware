...
{
    BL_Sem_t scan_sem = (BL_Sem_t)data;
    bl_os_sem_give(scan_sem);
}
...
static wps_scan_result_t wps_scan_(void)
{
    ...
    BL_Sem_t scan_sem = NULL;

    if (NULL == (scan_sem = bl_os_sem_create(0))) {
    ...
    while (bl_os_get_tick() - sm->start_tick < WPS_TIMEOUT_MS) {
        wifi_mgmr_scan(scan_sem, wps_scan_complete_);
        bl_os_sem_take(scan_sem, BL_0S_WAIT_FOREVER);
...
static void wifi_event_cb_(input_event_t *event, void *private_data)
{
    ...
    if (event->code == CODE_WIFI_ON_DISCONNECT) {
        ...
        bl_os_mq_send_wait(sm->event_queue, &msg, sizeof(wps_evq_msg_t), BL_0S_WAIT_FOREVER, 0);
    }
}

static void wps_timeout_cb_(void* xTimer)
{
    struct wps_sm *sm = gWpsSm;
...
static void wps_success_timer_cb_(void* xTimer)
{
...
static void wps_task_(void *pvParameters)
{
    ...
    uint32_t time_elapsed = bl_os_get_tick() - sm->start_tick;
    if (time_elapsed >= WPS_TIMEOUT_MS) {
        notify_user_(BL_WPS_EVENT_TIMEOUT, 0);
        goto error_out;
    }
    uint32_t timeout_timer_period = WPS_TIMEOUT_MS - time_elapsed;
    sm->timeout_timer = bl_os_timer_create(wps_timeout_cb_, sm);
    bl_os_timer_start_once(sm->timeout_timer, timeout_timer_period, 0);

    for (;;) {
        wps_evq_msg_t msg;
        bool exit_loop = false;
        if (0 != bl_os_mq_recv(sm->event_queue, &msg, sizeof(wps_evq_msg_t), BL_0S_WAIT_FOREVER)) {
            break;
        }

        switch (msg.event) {
        case WPS_EVQ_SUCCESS:
            sm->success_cb_timer = bl_os_timer_create(wps_success_timer_cb_, sm);
            bl_os_timer_start_once(sm->success_cb_timer, 1000, 0);
            break;
        ...
        case WPS_EVQ_DISCONNECTED:
            if (sm->timeout_timer) {
                bl_os_timer_delete(sm->timeout_timer, BL_0S_WAIT_FOREVER);
                sm->timeout_timer = NULL;
            }
            if (sm->success_cb_timer) {
                bl_os_timer_delete(sm->success_cb_timer, BL_0S_WAIT_FOREVER);
                sm->success_cb_timer = NULL;
            }
            if (sm->wps->state == WPS_FINISHED && sm->ap_cred_cnt == 1) {
                bl_wps_ap_credential_t *cred;
                prepare_stop_();
                if ((cred = bl_os_malloc(sizeof(*cred)))) {
...
static int wifi_station_wps_deinit_(void)
{
    ...
    if (sm->event_queue) {
        bl_os_mq_delete(sm->event_queue);
    }
...
bl_wps_err_t bl_wifi_wps_start(const struct bl_wps_config *config)
{
    ...
    if (!(sm->event_queue = bl_os_mq_creat(WPS_EVENT_QUEUE_CAPACITY, sizeof(wps_evq_msg_t)))) {
        ret = BL_WPS_ERR_MEMORY;
        goto ret;
    }
    ...
    if (0 != bl_os_task_create("wps", wps_task_, WPS_TASK_STACK_DEPTH, NULL, 10, &sm->wps_task_hdl)) {
        ret = BL_WPS_ERR_MEMORY;
        goto err;
    }
