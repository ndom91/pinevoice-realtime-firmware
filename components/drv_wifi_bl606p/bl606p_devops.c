...
static void wifi_sta_start_task(void *arg)
{
    ...
    if (0 != config->ssid[0]) {
        wifi_mgmr_sta_connect(sta_wifi_interface, config->ssid, strlen(config->password)?config->password:NULL, NULL, NULL, 0, 0);
    }
    LOGD(TAG, "%s\r\n", __func__);
...
static uint8_t g_is_sntp_task_running = false;
static const uint8_t retry_backoff_time[WIFI_SERVICE_RETRY_MAX_NUM]= {2, 2, 2, 4, 4, 4, 8, 8};
static const char *m_sntp_servaddr[SNTP_MAX_SERVERS] = {
        "cn.pool.ntp.org",
        "0.cn.pool.ntp.org",
        "1.cn.pool.ntp.org",
};
...
static void sntp_config_servaddr(void)
{
    ...
    for (i = 0; i < SNTP_MAX_SERVERS; i++) {
        if (0 != sntp_set_server(i, (char *)m_sntp_servaddr[i])) {
            LOGD(TAG, "set sntp server:%s failed\n", m_sntp_servaddr[i]);
        }
        else {
            LOGD(TAG, "set sntp server:%s successfully\n", m_sntp_servaddr[i]);
        }
    }
}
...
static bool sntp_start(void)
{
    ...
    while(1) {
        ...
        if (0 == sntp_get_time(&m_sntp_arg, &ntp_time)) {
            return true;
        } else if ((WIFI_STATE_CONNECTED_IP_GOT != wifi_state)
                   && (WIFI_STATE_CONNECTED_IP_GETTING != wifi_state)) {
            return false;
        } else {
            LOGD(TAG, "sntp retry:%d \n", round);
...
static void sntp_startup(void)
{
    aos_task_t  sntp_task;

    if(g_is_sntp_task_running == false) {
        aos_task_new_ext(&sntp_task, "sntp_task", wifi_sntp_handle, NULL, 4*1024, 30);
    } else {
        LOGD(TAG, "sntp task is already running.\n");
    }
}
