...
public:
  size_t ByteSizeLong() const override {
    return (6 * sizeof(int32_t) + kws_word_.size());
  }
  ...
  bool SerializeToArray(void* data, size_t size) const override {
    ...
    *(ptr + 5) = CX_HTOLE32(kws_word_.size());
    std::memcpy(ptr + 6, kws_word_.data(), kws_word_.size());
    return true;
  }
  ...
  bool ParseFromArray(const void* data, size_t size) {
    ...
    size_t word_size = CX_LE32TOH(*(ptr + 5));
    kws_word_.resize(word_size);
    std::memcpy(&kws_word_[0], ptr + 6, word_size);
    return true;
  }
