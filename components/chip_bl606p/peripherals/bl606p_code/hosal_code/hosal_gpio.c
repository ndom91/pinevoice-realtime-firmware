#include "peripherals_config.h"
#include <hosal_gpio.h>

#include <drv/common.h>
#include <bl_gpio.h>
#include <bl_irq.h>

typedef struct hosal_gpio_call_fuc_priv_t {
    csi_dev_t         gpio_dev;
    hosal_gpio_ctx_t *node;
    void *priv;
}hosal_gpio_call_fuc_priv;

#define GPIP_INT_STATE_OFFSET    (0x1a8)
static hosal_gpio_ctx_t *gpio_head = NULL;
static hosal_gpio_call_fuc_priv gpio_irq_func;

extern void bl_irq_register_with_ctx_yoc(int irqnum, void *handler, void *ctx);

int hosal_gpio_init(hosal_gpio_dev_t *gpio)
{
    if (gpio == NULL) {
        return -1;
    }

    switch (gpio->config) {
    case INPUT_PULL_DOWN:
        bl_gpio_enable_input(gpio->port, 0, 1);
        break;
    case INPUT_PULL_UP:
        bl_gpio_enable_input(gpio->port, 1, 0);
        break;
    case INPUT_HIGH_IMPEDANCE:
        bl_gpio_enable_input(gpio->port, 0, 0);
        break;
    case OUTPUT_PUSH_PULL:
        bl_gpio_enable_output(gpio->port, 1, 0);
        break;
    case OUTPUT_OPEN_DRAIN_NO_PULL:
        bl_gpio_enable_output(gpio->port, 0, 0);
        break;
    case OUTPUT_OPEN_DRAIN_PULL_UP:
        bl_gpio_enable_output(gpio->port, 0, 0);
        break;
    default:
        return -1;
    }
    return 0;
}

int hosal_gpio_output_set(hosal_gpio_dev_t *gpio, uint8_t value)
{
    if (gpio == NULL) {
        return -1;
    }
    bl_gpio_output_set(gpio->port, value ? 1 : 0);
    return 0;
}

int hosal_gpio_input_get(hosal_gpio_dev_t *gpio, uint8_t  *value)
{
    if (gpio == NULL) {
        return -1;
    }

    bl_gpio_input_get((GLB_GPIO_Type)gpio->port, value);
    return 0;
}

static int check_gpio_is_interrupt(int gpioPin)
{
    int ret = -1;

    ret = GLB_Get_GPIO_IntStatus(gpioPin);
    
    return ret;
#if 0
    int bitcount = 0;
    int reg_val = 0;

    bitcount = 1 << gpioPin;
    reg_val = *(int32_t *)(GLB_BASE + GPIP_INT_STATE_OFFSET);

    if ((bitcount & reg_val) == bitcount) {
        return 0;
    }
    return -1;
#else

#endif
}

static int exec_gpio_handler(hosal_gpio_ctx_t *node)
{
    bl_gpio_intmask(node->pin, 1);

    if (node->handle) {
        node->handle(node->arg);
        bl_gpio_int_clear(node->pin, 0);
        bl_gpio_intmask(node->pin, 0);
        return 0;
    }
    bl_gpio_intmask(node->pin, 0);

    return -1;
}

static void gpio_interrupt_entry(hosal_gpio_ctx_t *pstnode)
{
    int ret;
    hosal_gpio_ctx_t *node  = pstnode;

    while (node) {
        ret = check_gpio_is_interrupt(node->pin);
        if (ret == 1) {
            exec_gpio_handler(node);
        }

        node = node->next;
    }
    return;
}

static void gpio_interrupt_entry_yoc(void *pstnode)
{
    int ret;
    hosal_gpio_ctx_t *node  = ((hosal_gpio_call_fuc_priv *)pstnode)->node;

    while (node) {
        ret = check_gpio_is_interrupt(node->pin);
        if (ret == 1) {
            exec_gpio_handler(node);
        }

        node = node->next;
    }
    return;
}

int hosal_gpio_irq_set(hosal_gpio_dev_t *gpio, hosal_gpio_irq_trigger_t trigger_type, hosal_gpio_irq_handler_t handler, void *arg)
{
    if (NULL == gpio || gpio->port > GLB_GPIO_PIN_MAX || trigger_type > 3) {
        printf("hosal irq register paraments is not correct! \r\n");
    }
    hosal_gpio_ctx_t *node = NULL;
    hosal_gpio_ctx_t *node_f = NULL;
    node = (hosal_gpio_ctx_t *)pvPortMalloc(sizeof(hosal_gpio_ctx_t));
    if (!node) {
        printf("hosal irq ctx malloc failed \r\n");
        return -1;
    }
    node->handle     = handler;
    node->arg        = arg;
    node->pin        = gpio->port;
    node->intCtrlMod = 1;//GLB_GPIO_INT_CONTROL_ASYNC;                              // TODO :GLB_GPIO_INT_CONTROL_ASYNC is not define
    node->intTrigMod = trigger_type;
    if (!gpio_head) {
        gpio_head = node;
        node->next = NULL;
    } else {
        for (node_f = gpio_head; node_f != NULL; node_f = node_f->next) {
            if (node_f->pin == node->pin) {
#if 0
                memcpy(node_f, node, sizeof(hosal_gpio_ctx_t));// will crash beacause next field is NULL
#endif
                node_f->handle = node->handle;
                node_f->arg    = node->arg;
                node_f->intCtrlMod = node->intCtrlMod;
                node_f->intTrigMod = node->intTrigMod;
                vPortFree(node);
                break;
            }
        }
        if (node_f == NULL) {
            node->next = gpio_head;
            gpio_head = node;
        }
    }

    bl_gpio_intmask(gpio->port, 1);
//    bl_set_gpio_intmod(gpio->port, GLB_GPIO_INT_CONTROL_ASYNC, trigger_type);   // TODO:  mcu代码暂时没有GLB_GPIO_INT_CONTROL_ASYNC define
    bl_irq_register_with_ctx(GPIO_INT0_IRQn, gpio_interrupt_entry, gpio_head);
    bl_gpio_intmask(gpio->port, 0);
    bl_irq_enable(GPIO_INT0_IRQn);

    return 0;
}

int hosal_gpio_irq_set_yoc(hosal_gpio_dev_t *gpio, hosal_gpio_irq_trigger_t trigger_type, hosal_gpio_irq_handler_t handler, void *arg)
{
    if (NULL == gpio || gpio->port > GLB_GPIO_PIN_MAX || trigger_type > 3) {
        printf("hosal irq register paraments is not correct! \r\n");
    }
    hosal_gpio_ctx_t *node = NULL;
    hosal_gpio_ctx_t *node_f = NULL;
    GLB_GPIO_INT_Cfg_Type intCfg;

    node = (hosal_gpio_ctx_t *)pvPortMalloc(sizeof(hosal_gpio_ctx_t));
    if (!node) {
        printf("hosal irq ctx malloc failed \r\n");
        return -1;
    }
    node->handle     = handler;
    node->arg        = arg;
    node->pin        = gpio->port;
    node->intCtrlMod = 1;//GLB_GPIO_INT_CONTROL_ASYNC;                              // TODO :GLB_GPIO_INT_CONTROL_ASYNC is not define
    node->intTrigMod = trigger_type;

    intCfg.gpioPin = gpio->port;
    intCfg.trig = (GLB_GPIO_INT_TRIG_Type)trigger_type;
    intCfg.intMask = 1;

    if (!gpio_head) {
        gpio_head = node;
        node->next = NULL;
    } else {
        for (node_f = gpio_head; node_f != NULL; node_f = node_f->next) {
            printf("node_f->pin:%d, node->pin:%d\r\n", node_f->pin, node->pin);
            if (node_f->pin == node->pin) {
#if 0
                memcpy(node_f, node, sizeof(hosal_gpio_ctx_t));// will crash beacause next field is NULL
#endif
                node_f->handle = node->handle;
                node_f->arg    = node->arg;
                node_f->intCtrlMod = node->intCtrlMod;
                node_f->intTrigMod = node->intTrigMod;
                vPortFree(node);
                break;
            }
        }
        if (node_f == NULL) {
            node->next = gpio_head;
            gpio_head->next = NULL;
            gpio_head = node;
        }
    }
    gpio_irq_func.node = gpio_head;
    bl_gpio_intmask(gpio->port, 1);
    GLB_GPIO_Int_Init(&intCfg);
//    bl_set_gpio_intmod(gpio->port, GLB_GPIO_INT_CONTROL_ASYNC, trigger_type);   // TODO:  mcu代码暂时没有GLB_GPIO_INT_CONTROL_ASYNC define
    bl_irq_register_with_ctx_yoc(GPIO_INT0_IRQn, gpio_interrupt_entry_yoc, &gpio_irq_func);
    bl_gpio_intmask(gpio->port, 0);
    bl_irq_enable(GPIO_INT0_IRQn);

    return 0;
}

int hosal_gpio_irq_mask(hosal_gpio_dev_t *gpio, uint8_t mask)
{
    if (gpio  == NULL) {
        return -1;
    }
    bl_gpio_intmask(gpio->port, mask ? 1 : 0);

    return 0;
}

int hosal_gpio_finalize(hosal_gpio_dev_t *gpio)
{
    if (gpio == NULL) {
        return -1;
    }
    hosal_gpio_ctx_t *node = NULL;
    for (node = gpio_head; node != NULL; node = node->next) {
        if (node->pin == gpio->port) {
            vPortFree(node);
        }
    }
    if (node == NULL) {
        gpio_head = NULL;
    }
    bl_gpio_intmask(gpio->port, 1);
    bl_gpio_int_clear(gpio->port, 0);
    return 0;
}



