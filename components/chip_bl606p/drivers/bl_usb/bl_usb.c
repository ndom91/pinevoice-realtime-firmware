...
static int usb_vdma_write(uint8_t fifo_num, const uint8_t *data, uint32_t len, uint32_t timeout)
{
...
static int usb_vdma_read(uint8_t fifo_num, const uint8_t *data, uint32_t len, uint32_t timeout)
{
    uint32_t t = 0;
    USB_VDMA_Cfg_Type vdma_fifo_cfg;
...
int usb_dc_init(void)
{
    ...
    //csi_irq_attach(USB_IRQn, USBD_IRQHandler, (void *)&usb_hs_device);
    csi_irq_attach(USB_IRQn, USBD_IRQHandler, (void *)&g_usb0);
    csi_irq_enable(USB_IRQn);
...
int usbd_ep_open(const struct usb_dc_ep_cfg *ep_cfg)
{
    ...
    if (ep_cfg->ep_mps <= 512) {
        ...
        if (ep_cfg->ep_type == 0x01) {
            ...
            uint32_t tmpVal;
            tmpVal = BL_RD_WORD(USB_BASE + USB_DEV_INMPS1_OFFSET + 0x04 * (ep_idx - 1));
            tmpVal = BL_SET_REG_BITS_VAL(tmpVal, USB_TX_NUM_HBW_IEP1, 2);
            BL_WR_WORD(USB_BASE + USB_DEV_INMPS1_OFFSET + 0x04 * (ep_idx - 1), tmpVal);
 ...
 * @return 0 on success, negative errno code on fail.
 */
int usbd_ep_write(const uint8_t ep, const uint8_t *data, uint32_t data_len, uint32_t *ret_bytes, uint32_t timeout)
{
...
#ifdef CONFIG_USB_VDMA_ENABLE
        ret = usb_vdma_write(0, data, data_len, timeout);
#else
...
#ifdef CONFIG_USB_VDMA_ENABLE
        ret = usb_vdma_write(target_fifo_id, data, data_len, timeout);
#else
 ...
 * @return 0 on success, negative errno code on fail.
 */
int usbd_ep_read(const uint8_t ep, uint8_t *data, uint32_t data_len, uint32_t *read_bytes, uint32_t timeout)
{
    uint32_t read_len = 0;
    uint32_t target_fifo_id = 0;
    uint8_t ep_idx = USB_EP_GET_IDX(ep);
...
#ifdef CONFIG_USB_VDMA_ENABLE
            ret = usb_vdma_read(0, data, read_len, timeout);
#else
            ret = usb_dma_read(0, data, read_len, timeout);
#endif
            if (ret < 0) {
                return ret;
            }
...
#ifdef CONFIG_USB_VDMA_ENABLE
        ret = usb_vdma_read(target_fifo_id, data, read_len, timeout);
#else
