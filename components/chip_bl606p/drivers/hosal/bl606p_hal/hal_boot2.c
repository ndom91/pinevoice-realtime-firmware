...
#define blog_info_raw printf

//#define PARTITION_BOOT2_RAM_ADDR_ACTIVE (0x22057C00)
...
static void _dump_partition(void)
{
    ...
    blog_info_raw(" age %u;", part->ptTable.age);
    blog_info_raw(" crc32 0x%08X\r\n", (unsigned int)part->ptTable.crc32);

    blog_info_raw("idx  type device activeIndex     name   Address[0]  Address[1]  Length[0]   Length[1]   age\r\n");
...
int hal_boot2_init(void)
{
    ...
    blog_info_raw("[HAL] [BOOT2] Active Partition[%u]\r\n",
