...
static void http_client_handle(void *args)
{
    ...
    while (context->prov_enable == 1) {
        ...
        bool hasUser = false;
        if (strstr(socket_recv_buf, "wifiAccount")) {
            hasUser = true;
        }

        if (strstr(socket_recv_buf,"ssid"))
        {
            // get ssid:
            char *pos1 = NULL;
            char *pos2 = NULL;
            pos1 = strstr((char *)socket_recv_buf, "ssid:");
            if (pos1) {
                pos2 = strstr(pos1, "\n");
            }
            if (!pos1 || !pos2) {
            ...
            memset(ssid, 0, 64);
            memcpy(ssid, pos1 + strlen("ssid:"), pos2 - pos1 - strlen("ssid:"));
            // strcpy(ssid, pos1 + strlen("ssid:"));
            LOGD(TAG, "ssid:%s \n", ssid);
            // LOGD(TAG, "pos : %s \n", pos1);
            ...
            memset(pw, 0, 64);
            memcpy(pw, pos1 + strlen("pw:"), pos2 - pos1 - strlen("pw:"));
            // strcpy(pw, pos1 + strlen("pw:"));
            LOGD(TAG, "pw:%s \n", pw);
            // LOGD(TAG, "pos : %s \n", pos1);

            char* plain_pw = (char* )pwDecode(pw);
            // LOGD(TAG, "plain_pw:%s ", plain_pw);
            int i=0, len=strlen(plain_pw);
            for(i=0;i<len;i++)
            {
                if(plain_pw[i] < 0x20)
                {
            ...
            memset(pw, 0, 64);
            memcpy(pw, plain_pw, len);
            pw[len]='\0';
            aos_free(plain_pw);
            LOGD(TAG, "decrypt pw:%s \n", pw);

            // get thingname:
            pos1 = strstr(socket_recv_buf, "thingname:");

            memset(thingname, 0, 64);
            strcpy(thingname, pos1 + strlen("thingname:"));
            LOGD(TAG, "thingname:%s \n", thingname);
            // LOGD(TAG, "pos : %s \n", pos1);
            // LOGD(TAG, "thingname.len %d pos:%d\n", strlen(thingname), strlen(pos1));

            // send mac to APP: /* get wifi_mac (Upper Letter)*/
            uint8_t mac[6] = {0};
            bl_efuse_read_mac_smart(1, mac, 0);
            sprintf(send_buf, "MAC:%02X%02X%02X%02X%02X%02X", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
            sendall(iNewSockFD, send_buf, strlen(send_buf), 0);
            LOGD(TAG, "\n\n\nsend wifi-mac: send_buf=%s\n\n\n",send_buf);

            // save / trans ssid.psd.thingname...
            aos_kv_setstring("wifiSSID", ssid);
            aos_kv_setstring("wifiPSK", pw);
            aos_kv_setstring("usrThingName", thingname);
            strncpy((char *)context->res.ssid, ssid, MAX_SSID_SIZE);
            context->res.ssid[MAX_SSID_SIZE] = 0;
            strncpy((char *)context->res.password, pw, MAX_PASSWD_SIZE);
            context->res.password[MAX_PASSWD_SIZE] = 0;
            // then wait for recving "OK"
        }

        if (strstr(socket_recv_buf, "OK"))
        {
...
static int http_server(struct ap_prov_context *context)
{
    ...
    sLocalAddr.sin_port        = htons(APP_SERVER_PORT);
...
void pw_server_start()
{
    LOGE(TAG, "pw_server_start.");
    g_softap_prov_ctx.prov_enable = 1;
    http_server(&g_softap_prov_ctx);
    // prov_thread_start(&g_softap_prov_ctx);
}
...
#define snprintf_nowarn(...) (snprintf(__VA_ARGS__) < 0 ? abort() : (void)0)
    ...
    char psk[]="12345678";
    snprintf_nowarn(config.password, sizeof(config.password), "%s", psk);
    config.password[sizeof(config.password) - 1] = 0;
    LOGI(TAG, "Start AP: ssid.%s password.%s", config.ssid, config.password);
