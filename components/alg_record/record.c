...
static int data_ready(void *arg, void *data, size_t len)
{
    unsigned   flags   = 0;
...
int rec_start_ws(const char *url, const char *save_name, int rb_size, int chncnt)
{
    ...
    for (int i = 0; i < grpcnt; i++) {
        ...
        snprintf(url_from, sizeof(url_from), "rbuf://handle=%llu", (long long unsigned)(&rec_mgr->ringbuffer));
