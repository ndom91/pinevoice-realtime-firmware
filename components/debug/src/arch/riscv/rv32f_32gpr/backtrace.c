...
static int backtraceFindLROffset(char *LR, int (*print_func)(const char *fmt, ...))
{
    ...
    if (print_func != NULL) {
        k_int64tostr((long)LR_indeed - offset, &s_panic_call[14]);
...
static int backtraceFromStack(long **pSP, char **pPC, long **pLR,
                              int (*print_func)(const char *fmt, ...))
{
    if (BT_CHK_PC_AVAIL(*pPC) == 0) {
        return -1;
    	//print_func("pPC=pLR:  %p \r\nbacktrace : %p\r\n", *pPC, *pLR);
    	//*pPC = (char *)(*pLR);
    }
...
int backtrace_now(int (*print_func)(const char *fmt, ...))
{
    ...
    for (lvl = 0; lvl < BT_LVL_LIMIT; lvl++) {
        ret = backtraceFromStack(&SP, &PC, &LR, print_func);
        if (ret != 0) {
            break;
        }
    }
                     ...
                     int (*print_func)(const char *fmt, ...))
{
    ...
    for (lvl = 0; lvl < BT_LVL_LIMIT; lvl++) {
        ret = backtraceFromStack(&bt_sp, &bt_pc, (long **)&bt_lr, print_func);
        if (ret != 0) {
            break;
        }
    }
...
void backtrace_task(void *task, int (*print_func)(const char *fmt, ...))
{
    ...
    print_func("========== Call stack ==========\r\n");
    k_int64tostr((long)BT_PC2ADDR(PC), &panic_call[14]);
    if (print_func != NULL) {
...
int backtrace_now_get(void *trace[], int size, int offset)
{
    ...
    for (lvl = 0; lvl < BT_LVL_LIMIT; lvl++) {
        ret = backtraceFromStack(&SP, &PC, &LR, NULL);
        if (ret != 0) {
            break;
        }
                      ...
                      int (*print_func)(const char *fmt, ...))
{
    ...
    for (lvl = 0; lvl < BT_LVL_LIMIT; lvl++) {
        ret = backtraceFromStack(&pSP, &PC, (long **)&LR, print_func);
        if (ret != 0) {
            break;
        }
    }
