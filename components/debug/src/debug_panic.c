...
static int g_debug_panic_cli = 1;
...
void stack_dump(cpu_stack_t *stack_origin, uint32_t size)
{
    uint32_t zero_cnt, zero_prt, idx;
    uint32_t *stack = (uint32_t *)stack_origin;
    size = size * (sizeof(cpu_stack_t) / sizeof(uint32_t));
    ...
    for (idx = 0, zero_cnt = 0, zero_prt = 0; idx < size; idx += 4) {
        ...
        panic_print("(0x%08X): 0x%08X 0x%08X 0x%08X 0x%08X\r\n",
                    &stack[idx], stack[idx], stack[idx + 1], stack[idx + 2], stack[idx + 3]);
        zero_cnt = 0;
        zero_prt = 0;
    }
}
