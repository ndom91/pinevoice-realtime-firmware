...
static int compare_str_mem(button_evt_t *p1, event_pool_elem_t *p2, size_t len)
{   
    int ret = 0;

    if (p1->button_id != p2->button_id) {
        ret++;
    }
    if (p1->press_type != p2->press_type) {
        ret++;
    }
    if (p1->press_time != p2->press_time) {
        ret++;
    }
...
static int event_param_check(button_evt_t *buttons, int button_count)
{
    ...
    slist_for_each_entry(&g_button_srv.event_node_head, e_node, event_node_t, next) {
        if ((button_count == e_node->button_count) && (compare_str_mem(e_node->buttons, buttons, sizeof(button_evt_t)*button_count) == 0)) {
            return -1;         
        }
    }
...
static int event_pool_reset(void)
{
    ...
    slist_for_each_entry(&g_button_srv.event_node_head, node, event_node_t, next) {
        if (node->button_count == g_button_srv.event_pool_depth) {
            int ret = compare_str_mem(node->buttons, g_button_srv.event_pool, \
...
static event_node_t *event_find(event_pool_elem_t *elem)
{
    ...
    slist_for_each_entry(&g_button_srv.event_node_head, e_node, event_node_t, next) {
        if ((e_node->button_count == 1) && (0 == compare_str_mem(elem, e_node->buttons, sizeof(event_pool_elem_t)))) {
            return e_node;
        }
    }
