...
uint32_t g_count_channel_put_message = 0;
uint32_t g_count_channel_put_message_done = 0;
uint32_t g_count_channel_get_message = 0;
uint32_t g_count_channel_get_message_done = 0;
...
int channel_get_message(channel_t *ch, void *msg, int len, uint32_t timeout_ms)
{
    ...
    ret = csi_mailbox_receive(handle, ch->des_id, msg, len);
