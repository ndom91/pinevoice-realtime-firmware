...
static int ipc_channel_send(ipc_t *ipc, phy_data_t *msg, int timeout_ms)
{
    ...
    if (aos_event_get(&ipc->evt, IPC_WRITE_EVENT, AOS_EVENT_OR_CLEAR, &flag, timeout_ms) != 0) {
        printf("%s timeout\r\n", __func__);
        aos_log_hexdump("ipc0", (volatile int *)0x2000a800, 64);
        aos_log_hexdump("ipc1", (volatile int *)0x2000a840, 64);
        aos_log_hexdump("ipc2", (volatile int *)0x30005000, 64);
    }
...
static int ipc_channel_recv(ipc_t *ipc, phy_data_t *msg, int timeout_ms)
{
    ...
    if ((ipc->seq_bake + 1) != msg->seq) {
        LOGI(TAG, "ipc lost(%s) cur:%d msg->seq:%d", aos_task_get_name(&ipc->thread), ipc->seq_bake, msg->seq);
        ipc->seq_bake = msg->seq;
    } else {
        ipc->seq_bake ++;
    }
...
static int transfer_send(service_t *ser, message_t *msg, int timeout_ms)
{
    ...
    //int flag;
    //aos_event_get(&ipc->send_enable, 0x1, AOS_EVENT_OR_CLEAR, &flag, AOS_WAIT_FOREVER);
    //aos_event_set(&ipc->send_enable, 0x1, AOS_EVENT_OR);
...
static int transfer_recv(service_t *ser, message_t *msg, int timeout_ms)
{
    ...
    if (dispatch->queue != phy_msg.sem) {
        printf("dispatch->queue:%p phy_msg.sem:%p\r\n", dispatch->queue, phy_msg.sem);
    }
    if (total_len > 0) {
...
extern uint32_t g_isr_ipc_cnt1    ;
extern uint32_t g_isr_ipc_cnt2    ;
extern uint32_t g_isr_ipc_ackcnt1 ;
extern uint32_t g_isr_ipc_ackcnt2 ;
extern uint32_t g_isr_ipc_datacnt1;
extern uint32_t g_isr_ipc_datacnt2;

void debug_printf_ipc_count(void)
{
#if defined(CONFIG_CPU_E907)
        printf("put:%ld,%ld,%ld,%ld; isr:%ld,%ld,%ld,%ld,%ld,%ld\r\n",
#else if defined(CONFIG_CPU_C906)
        printf("put:%d,%d,%d,%d; isr:%d,%d,%d,%d,%d,%d\r\n",
#endif
...
int ipc_send_enable(ipc_t *ipc, uint8_t enable)
{
    int ret = 0;
    ...
    if (enable) {
        ...
        int flag;
        //ret = aos_event_get(&ipc->send_enable, 0x1, AOS_EVENT_OR_CLEAR, &flag, 0);
        ret = aos_event_get(&ipc->evt, IPC_WRITE_EVENT, AOS_EVENT_OR_CLEAR, &flag, 0);
    }
    return ret;
}
...
#define IPC_NAME_MAX_LEN 16
    ...
    if (ipc->ch != NULL) {
        ...
        aos_check(!ret, ENOME);
#endif
        //ret = aos_event_new(&ipc->send_enable, 0x1);
        //aos_check(!ret, ENOME);
        return ipc;
    }
...
int ipc_add_service(ipc_t *ipc, int service_id, ipc_process_t cb, void *priv)
{
    ...
    if (ser) {
        ...
        aos_event_set(&ipc->evt, IPC_READ_EVENT, AOS_EVENT_OR);
        return 0;
    }
