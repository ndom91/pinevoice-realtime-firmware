...
#define BT_HAL_DBG(...)  LOGD("BTHAL", __VA_ARGS__)
...
static bool init_a2dp_flag = false;
static bool link_loss_evt = false;
...
{   
    ...
    return ao_control(ao, cmd, &speed, sizeof(speed));
}
...
static void tg_bt_a2dp_connect_remote(void *args)
{
    struct bt_a2dp *a2dp = NULL;

    if (t_bt_def_conn && init_a2dp_flag){
        BT_HAL_DBG("%s, state:%d\n", __func__, t_bt_def_conn->state);
        if(t_bt_def_conn->state == BT_CONN_CONNECTED){
        ...
        init_a2dp_flag = false;
    }
}

static void bt_hal_evt_cb(void *args)
{
    int8_t *addr = (int8_t *)args;

    BT_HAL_DBG("%s\n", __func__);
    if(t_a2dp_cb){
        bt_prf_a2dp_cb_param_t a2dp_cb_param = {0};

        a2dp_cb_param.conn_stat.state = BT_PRF_A2DP_CONNECTION_STATE_DISCONNECTED_ABNORMAL;
        memcpy(a2dp_cb_param.conn_stat.peer_addr.val,addr,6);
        link_loss_evt = false;
...
static void t_remote_name_callback(const char *name)
{
    ...
    if (t_bt_event_cb)
    {
        ...
        auth_evt.auth_cmpl.device_name = t_remote_name;
...
static void t_bt_connected(struct bt_conn *conn, u8_t err)
{
    ...
    if (err) {
        ...
        */
        if(err == 0x4 || err == 0xb){
            if(t_a2dp_cb){
                bt_prf_a2dp_cb_param_t a2dp_cb_param = {0};
                memcpy(a2dp_cb_param.conn_stat.peer_addr.val, conn->br.dst.val, BT_STACK_BD_ADDR_LEN);
    ...
    if(init_a2dp_flag){
        aos_post_delayed_action(1000, tg_bt_a2dp_connect_remote, NULL);
    }
    ...
    extern int tg_wifi_ps_mode_entry(void);
...
static void t_bt_disconnected(struct bt_conn *conn, u8_t reason)
{
    ...
    if(link_loss_evt){
        aos_post_delayed_action(300, bt_hal_evt_cb, (void *)addr);
    }
}
...
bt_stack_status_t bt_stack_set_name(const char *name)
{
    ...
    err = bt_write_local_name((char *)name);
    if (!err) {
...
bt_stack_status_t bt_prf_hfp_hf_audio_connect(bt_dev_addr_t *peer_addr)
{
    ...
    conn = bt_conn_create_sco(&sco_addr);
    if (!conn) {
...
bt_stack_status_t bt_prf_hfp_hf_answer_call(void)
{
    ...
    err = bt_hfp_hf_send_cmd(t_bt_def_conn, BT_HFP_HF_ATA);
    if (err) {
...
bt_stack_status_t bt_prf_hfp_hf_reject_call(void)
{
    ...
    err = bt_hfp_hf_send_cmd(t_bt_def_conn, BT_HFP_HF_AT_CHUP);
    if (err) {
