...
static void connected(struct bt_conn *conn, u8_t err)
{
    ...
    event_data.conn_handle = conn->handle;
    if (err) {
        event_data.err = err;
...
int gatt_read_handle(void *conn, const void *attr, void *buf, uint16_t len, uint16_t offset)
{
    ...
    tlog("");
    event_data.conn_handle = ((struct bt_conn*)conn)->handle;
...
int ble_stack_gatt_registe_service(gatt_service *s, gatt_attr_t attrs[], uint16_t attr_num)
{
    ...
    for (i = 0; i < attr_num; i++) {
        if (UUID_EQUAL(attrs[i].uuid, UUID_GATT_PRIMARY) || UUID_EQUAL(attrs[i].uuid, UUID_GATT_SECONDARY)
            ...
            val->cfg_changed = gatt_cfg_changed;
                          ...
                          struct bt_gatt_discover_params *params)
{
    ...
    if (attr == NULL) {
        ...
        event_data.comp.conn_handle = conn->handle;
    ...
    if (params->type == BT_GATT_DISCOVER_PRIMARY) {
        ...
        event_data.svc.conn_handle = conn->handle;
        ...
        event_data.svc_inc.conn_handle = conn->handle;
        ...
        event_data.char_c.conn_handle = conn->handle;
        ...
        event_data.char_des.conn_handle = conn->handle;
