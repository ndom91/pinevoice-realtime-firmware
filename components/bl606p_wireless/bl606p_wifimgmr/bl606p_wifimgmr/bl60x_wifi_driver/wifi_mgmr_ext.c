...
int wifi_mgmr_scan_result_sort();
...
{
    int i, j, idx = -1;
    wifi_mgmr_scan_item_t *scan, scan_tmp;

    for (i = 0; i < WIFI_MGMR_SCAN_ITEMS_MAX; i++) {
    ...
    if (idx == WIFI_MGMR_SCAN_ITEMS_MAX) {
        return 0;
    }

    for (i = idx; i < WIFI_MGMR_SCAN_ITEMS_MAX - 1; i++) {
        for (j = idx; j < WIFI_MGMR_SCAN_ITEMS_MAX - 1; j++){
            if (wifiMgmr.scan_items[j].rssi < wifiMgmr.scan_items[j+1].rssi) {
                memcpy(&scan_tmp, &wifiMgmr.scan_items[j], sizeof(wifi_mgmr_scan_item_t));
                memcpy(&wifiMgmr.scan_items[j], &wifiMgmr.scan_items[j+1], sizeof(wifi_mgmr_scan_item_t));
                memcpy(&wifiMgmr.scan_items[j+1], &scan_tmp, sizeof(wifi_mgmr_scan_item_t));
            }
        }
    }
...
int wifi_mgmr_all_ap_scan_adv(wifi_mgmr_ap_item_t **ap_ary, uint32_t *num, wifi_mgmr_scan_arg_t *scan_arg)
{
    uint32_t counter = 0;
    wifi_mgmr_ap_item_t *ap_ary_p;
    ...
    if (scan_arg) {
        wifi_mgmr_scan_adv(ap_ary_p, cb_scan_complete, scan_arg->channels, scan_arg->channel_num, scan_arg->bssid, scan_arg->ssid, SCAN_ACTIVE, 0);
    } else {
        wifi_mgmr_scan(ap_ary_p, cb_scan_complete);//trigger the scan
    }
