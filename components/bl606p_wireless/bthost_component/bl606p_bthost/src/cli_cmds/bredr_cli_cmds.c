...
static void bredr_get_address(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv);
...
static void bredr_switch_link_role(char *p_write_buffer, int write_buffer_len, int argc, char **argv);
static void bredr_link_policy_setting(char *p_write_buffer, int write_buffer_len, int argc, char **argv);
...
#if CONFIG_BT_PBAP
static void pbap_connect(char *p_write_buffer, int write_buffer_len, int argc, char **argv);
static void bt_pbap_downloab_pb(char *p_write_buffer, int write_buffer_len, int argc, char **argv);
#endif

const static struct cli_command bredr_cmd_set[] = {
    #if PCM_PRINTF
    {"pcm", "", pcm},
    #endif
    {"bredr_init", "", bredr_init},
    {"bredr_get_addr","",bredr_get_address},
    ...
    {"bredr_set_lp","",bredr_link_policy_setting},
    {"bredr_switch_role","",bredr_switch_link_role},
    ...
    #if CONFIG_BT_HFP
    {"hfp_conn", "", hfp_connect},
    #endif

    #if CONFIG_BT_PBAP
    {"pbap_connect", "", pbap_connect},
    {"pbap_download_pb", "", bt_pbap_downloab_pb},
    #endif
};
...
static void bredr_connected(struct bt_conn *conn, u8_t err)
{
    ...
    printf("bredr connected: %s handle 0x%x\r\n", addr,conn->handle);
...
static void bredr_link_policy_setting(char *p_write_buffer, int write_buffer_len, int argc, char **argv)
{
    ...
    get_uint8_from_string(&argv[2], &setting);
    err = bt_link_policy_setting(handle,setting);
    if(err){
        printf("Failed to set link policy\r\n");
    }
}
...
static void bredr_switch_link_role(char *p_write_buffer, int write_buffer_len, int argc, char **argv)
{
    ...
    get_uint8_from_string(&argv[2], &role);
    err = bt_switch_link_role(&peer_addr,role);
    if(err){
        printf("Failed to switch link role\r\n");
    }
}
...
static void hfp_connect(char *p_write_buffer, int write_buffer_len, int argc, char **argv)
{
    ...
    extern int bt_hfp_send_connect(struct bt_conn *conn);
    err = bt_hfp_send_connect(default_conn);
    if(err) {
        printf("hfp connect failed, err: %d\n", err);
    } else {
...
static void pbap_connect(char *p_write_buffer, int write_buffer_len, int argc, char **argv)
{
    ...
    err = bt_pbab_start_conn(default_conn);
    if(err) {
        printf("pbap connect failed, err: %d\n", err);
    } else {
        printf("pbap connect pending.\n");
    }
}
...
static void bt_pbap_downloab_pb(char *p_write_buffer, int write_buffer_len, int argc, char **argv)
{
    ...
    get_uint8_from_string(&argv[2], &prop);
    
    bt_pbap_client_download_phonebook(default_conn,prop,type);
}
#endif
int bredr_cli_register(void)
{
    ...
    ret = aos_cli_register_commands(bredr_cmd_set, sizeof(bredr_cmd_set)/sizeof(struct cli_command));
    if (ret != 0) {
        return ret;
    }
