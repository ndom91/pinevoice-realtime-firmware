...
#define BT_HCI_SWITCH_LINK_ROLE                 BT_OP(BT_OGF_LINK_POLICY,0x000b)
struct bt_hci_switch_role {
	bt_addr_t bdaddr;
	u8_t   role;
} __packed;

#define BT_HCI_LINK_POLICY_SET                  BT_OP(BT_OGF_LINK_POLICY,0x000d)
struct bt_hci_link_policy_setting {
	uint16_t handle;
	uint16_t link_policy_setting;
} __packed;
...
#define BT_HCI_EVT_DBG_OFFSET        0x02
#endif
...
#define BT_HCI_VS_BT_DEBUG_NOTIFY_SCH_TIME_TO_HOST   0x05
struct bt_hci_dbg_notify_sch_time_to_host_evt {
    u8_t             activity;
    u8_t             mode;
    u32_t            time;
    u32_t            args1;
