...
#endif /* CONFIG_BT_ECC */
typedef void (*bt_dbg_evt_cb_t)(const struct bt_hci_dbg_notify_sch_time_to_host_evt *evt);
static bt_dbg_evt_cb_t dbg_evt_cb;
...
int bt_link_policy_setting(uint16_t conn_handle,uint16_t link_policy_set)
{
	struct bt_hci_link_policy_setting *cp;
	struct bt_conn *conn;
	struct net_buf *buf;

	conn = bt_conn_lookup_handle(conn_handle);
	if (!conn) {
		BT_ERR("Can't find conn for handle %u", conn_handle);
		return -ENOTCONN;
	}

	buf = bt_hci_cmd_create(BT_HCI_LINK_POLICY_SET,
				sizeof(*cp));
	if (!buf) {
		return -ENOBUFS;
	}
	...
	cp->link_policy_setting = link_policy_set;

	return bt_hci_cmd_send(BT_HCI_LINK_POLICY_SET, buf);

}

int bt_switch_link_role(const bt_addr_t *bdaddr,uint8_t role)
{
	struct bt_hci_switch_role *cp;
	struct bt_conn *conn;
	struct net_buf *buf;
	...
	buf = bt_hci_cmd_create(BT_HCI_SWITCH_LINK_ROLE,
				sizeof(*cp));
	if (!buf) {
		return -ENOBUFS;
	}
	...
	return bt_hci_cmd_send(BT_HCI_SWITCH_LINK_ROLE, buf);
}
#endif /* CONFIG_BT_BREDR */
...
static void hci_encrypt_change(struct net_buf *buf)
{
	...
	if (conn->type == BT_CONN_TYPE_BR) {
		...
		if (IS_ENABLED(CONFIG_BT_SMP)) {
			...
			struct bt_hci_cp_write_auth_payload_timeout *auth_timeout;

			buf = bt_hci_cmd_create(BT_HCI_OP_WRITE_AUTH_PAYLOAD_TIMEOUT,
						sizeof(*auth_timeout));
			if (!buf) {
				return;
			}

			auth_timeout = net_buf_add(buf, sizeof(*auth_timeout));
			auth_timeout->handle = conn->handle;
			auth_timeout->auth_payload_timeout = 0xBB8;

			err = bt_hci_cmd_send_sync(BT_HCI_OP_WRITE_AUTH_PAYLOAD_TIMEOUT, buf,
						   NULL);
			if (err) {
				return;
			}
...
int bt_hci_register_dbg_evt_cb(bt_dbg_evt_cb_t cb)
{
	dbg_evt_cb = cb;
	return 0;
}

static void bt_dbg_evt(struct net_buf *buf)
{
	struct bt_hci_dbg_notify_sch_time_to_host_evt *evt = (void *)buf->data;
	//BT_WARN("activity 0x%x,mode 0x%x,time 0x%x,args1 0x%x,args2 0x%x",evt->activity,
	//        evt->mode,evt->time,evt->args1,evt->args2);
...
static const struct event_handler vendor_event[] = {
	EVENT_HANDLER(BT_HCI_VS_BT_DEBUG_NOTIFY_SCH_TIME_TO_HOST, bt_dbg_evt,
		      sizeof(struct bt_hci_dbg_notify_sch_time_to_host_evt)),
};
...
int bt_enable(bt_ready_cb_t cb)
{
    ...
    if (IS_ENABLED(CONFIG_BT_SETTINGS)) {
		...
		// TODO: The HCI command here times out, so let's skip it. It seems that after BLE is loaded it is all good. - gamiee
		// bt_set_name(CONFIG_BT_DEVICE_NAME);
	}
#endif
...
k_thread_create(&tx_thread_data, "hci_tx_thread",
	...
	extern int bt_monitor_start(void);
	bt_monitor_start();
#endif
	if (IS_ENABLED(CONFIG_BT_TINYCRYPT_ECC)) {
		bt_hci_ecc_init();
...
extern struct net_buf_pool pbap_pool;
#endif
#endif
...
#if defined(CONFIG_BT_BREDR)
int bt_write_local_name(const char *name)
{
	int err;
	struct net_buf *buf;
	struct bt_hci_write_local_name *name_cp;
...
int BTBredrSetA2dpStreamStatus( uint16_t conHandle,uint8_t status )
{
    struct bt_hci_cp_vs_set_a2dp_status *a2dpStatus;
    struct net_buf *buf;

    buf = bt_hci_cmd_create( HCI_VS_BT_A2DP_STATUS_CMD_OPCODE, sizeof(*a2dpStatus) );
    if( !buf )
    {
        return -ENOBUFS;
    }

    a2dpStatus = net_buf_add( buf, sizeof(*a2dpStatus) );
    memset( a2dpStatus, 0, sizeof(*a2dpStatus) );

    a2dpStatus->conhdl = conHandle;
    a2dpStatus->status = status;

    return bt_hci_cmd_send_sync( HCI_VS_BT_A2DP_STATUS_CMD_OPCODE, buf, NULL );

}

int BTBredrCoexSetA2dpDuty( uint8_t DutyValue        )
{
    struct hci_vsc_set_a2dp_duty_cycle_cmd *a2dpDuty;
    struct net_buf *buf;

    buf = bt_hci_cmd_create( HCI_VS_BT_A2DP_DUTY_CYCLE_CMD_OPCODE, sizeof(*a2dpDuty) );
    if( !buf )
    {
        return -ENOBUFS;
    }

    a2dpDuty = net_buf_add( buf, sizeof(*a2dpDuty) );
    memset( a2dpDuty, 0, sizeof(*a2dpDuty) );

    a2dpDuty->duty_cycle = DutyValue;
