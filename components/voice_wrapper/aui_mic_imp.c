...
#define FRAME_SIZE      ((16000 / 1000) * (16 / 8) * 10)           /* 640 */
...
static int _event_hdl(mic_t *mic, rpc_t *rpc)
{
    ...
    switch (param->evt_id) {
        ...
        case MIC_EVENT_SESSION_WWV:
            if (mic->cb) {
                mic->cb(mic->source, MIC_EVENT_SESSION_WWV, (void *)param->ival, 0);
            } 
            break;
        ...
        case MIC_EVENT_SESSION_DOA:
            if (mic->cb) {
                mic->cb(mic->source, MIC_EVENT_SESSION_DOA, (void *)param->ival, 0);
            }
            break;
        default:
            break;
...
int aui_mic_event_register(aui_mic_evt_t evt_cb)
{
    g_mic.cb = evt_cb;
    return 0;
}
