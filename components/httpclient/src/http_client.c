...
static http_errors_t http_client_mysend_post_data(http_client_handle_t client, my_post_data_t *post_data);
...
void http_client_set_post_len(http_client_handle_t client, int post_len)
{
    client->post_len = post_len;
}
...
http_errors_t http_client_myperform(http_client_handle_t client, my_post_data_t *post_data)
{
    http_errors_t err;
    do
    {
        ...
        case HTTP_STATE_CONNECTED:
            ...
            /* falls through */
        case HTTP_STATE_REQ_COMPLETE_HEADER:
            if ((err = http_client_mysend_post_data(client, post_data)) != WEB_OK)
            {
                if (client->is_async && errno == EAGAIN)
                {
                    return ERR_HTTP_EAGAIN;
