...
def get_mem_info(map_file):
    ...
    with open(map_file, 'r') as f:
        ...
        mem_map_list = re.findall('Linker script and memory map([\s\S]+?)OUTPUT\(', s)
        ...
        for module in modules:
            ...
            map_lines.append('| %-40s | %-8d  | %-8d | %-8d | %-8d | %-8d | %-8d |'%(re.sub('\.[ao]','',module)[:40],rom_size,ram_size, region_size_module['FLASH'], region_size_module['OCRAM'], region_size_module['WRAM'], region_size_module['PSRAM']))

    print '\n                                        E907 MEMORY MAP                                                  '
    print '|=============================================================================================================|'
    print '| %-40s | %-8s  | %-8s | %-8s | %-8s | %-8s | %-8s |'%('MODULE','ROM','RAM','FLASH','OCRAM','WRAM', 'PSRAM')
    print '|=============================================================================================================|'
    for line in map_lines:
        print line
    print '|=============================================================================================================|'
    print '| %-40s | %-8d  | %-8d | %-8d | %-8d | %-8d | %-8d |'%('TOTAL (bytes)', total_rom, total_ram, region_size['FLASH'], region_size['OCRAM'], region_size['WRAM'], region_size['PSRAM'])
    print '|=============================================================================================================|'
