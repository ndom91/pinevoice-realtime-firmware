...
{
    if ((task_handle == NULL) || (stack_size % sizeof(cpu_stack_t) != 0) || ((stack_size == 0) && (stack == NULL)) || prio <= KPRIO_IDLE || prio > KPRIO_REALTIME7) {
        return -EINVAL;
    }
    ...
    if (name) {
        ret = krhino_task_dyn_create((ktask_t **)task_handle, name, arg, prio_trans, time_quanta, stack_size / sizeof(cpu_stack_t), task, AUTORUN);
    } else {
        ret = krhino_task_dyn_create((ktask_t **)task_handle, "user_task", arg, prio_trans, time_quanta, stack_size / sizeof(cpu_stack_t), task, AUTORUN);
    }
...
uint32_t csi_kernel_task_get_stack_size(k_task_handle_t task_handle)
{
    ...
    ktask_t *handle = (ktask_t *)task_handle;
    return sizeof(cpu_stack_t) * handle->stack_size;
}
...
uint32_t csi_kernel_task_list(k_task_handle_t *task_array, uint32_t array_items)
{
    ...
    for (tmp = taskhead->next; tmp != taskend; tmp = tmp->next) {
        ...
        printf("\n%s:\n\t    state %d, pri %d, stack: total %p, free %p\n",
               task->task_name, task->task_state, task->prio, sizeof(cpu_stack_t) * task->stack_size, sizeof(cpu_stack_t) * task_free);
#endif
    }
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kstat_t ret = krhino_timer_dyn_create((ktimer_t **)&tmr, "UserTmr", (timer_cb_t)tmr_adapt_cb, first, round, get_arg, 0);
...
k_status_t csi_kernel_timer_del(k_timer_handle_t timer_handle)
{
    ...
    kstat_t ret = krhino_timer_dyn_del((ktimer_t *)timer_handle);
...
k_status_t csi_kernel_timer_start(k_timer_handle_t timer_handle, uint32_t ticks)
{
    ...
    tick_t round;
    ktimer_t *handle = (ktimer_t *)timer_handle;
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kevent_t *event_handle;
    kstat_t ret = krhino_event_dyn_create((kevent_t **)&event_handle, "UserEvent", 0);
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kstat_t ret = krhino_event_dyn_del(ev_handle);
...
{
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kmutex_t *mutex_handle;
    kstat_t ret = krhino_mutex_dyn_create((kmutex_t **)&mutex_handle, "UserMutex");
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kstat_t ret = krhino_mutex_dyn_del(mutex_handle);
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    ksem_t *sem_handle;
    kstat_t ret = krhino_sem_dyn_create((ksem_t **)&sem_handle, "UserSem", initial_count);
...
#if (RHINO_CONFIG_KOBJ_DYN_ALLOC > 0)
    kstat_t ret = krhino_sem_dyn_del(sem_handle);
...
k_mpool_handle_t csi_kernel_mpool_new(void *p_addr, int32_t block_count, int32_t block_size)
{
    ...
    kstat_t ret = krhino_mblk_pool_init(handle, "UserMp", p_addr, block_count * block_size);
...
#if (RHINO_CONFIG_MM_BLK > 0)
    return krhino_mblk_alloc(mp_handle, size);
#else
    return NULL;
#endif
}
...
k_status_t csi_kernel_msgq_put(k_msgq_handle_t mq_handle, const void *msg_ptr, uint8_t front_or_back, int64_t timeout)
{
...
k_status_t csi_kernel_msgq_get(k_msgq_handle_t mq_handle, void *msg_ptr, int64_t timeout)
{
...
k_status_t csi_kernel_mm_dump(void)
{
    ...
    dumpsys_mm_info_func(KMM_ERROR_UNLOCKED);
#endif
